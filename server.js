...
// Inicialización por defecto de los estados
...
// Función nativa para realizar pings reales sobre los 47 enlaces
function executePing(ip) {
    return new Promise((resolve) => {
        // Ejecución nativa del comando ping (compatible con Linux/Windows detectando la plataforma)
...
// Auditoría síncrona/asíncrona en lotes para verificar los 47 enlaces en tiempo real de forma continua
async function runNetworkPingAudit() {
    console.log("[NOC AUDIT] Ejecutando escaneo ICMP en tiempo real sobre los 47 enlaces...");
    const auditPromises = instituciones.map(async (inst) => {
        const result = await executePing(inst.ip);
        statusCache[inst.amie] = result;
    });
    await Promise.all(auditPromises);
    console.log("[NOC AUDIT] Escaneo de conectividad finalizado con éxito.");
}
...
app.listen(PORT, async () => {
    console.log(`================================================================`);
    console.log(` CENTRO DE OPERACIONES DE RED - 07D06 MINEDEC FUNCIONANDO `);
    console.log(` Puerto local activo: http://localhost:${PORT} `);
    console.log(`================================================================`);
    
    // Ejecución inicial inmediata
    runNetworkPingAudit();
    
    // Intervalo de escaneo continuo cada 30 segundos
    setInterval(runNetworkPingAudit, 30000);
});
